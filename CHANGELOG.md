...
- The rate limit on password-reset requests keeps its memory bounded: it
  holds at most 10,000 callers at once and does a constant amount of work per
  request, so a client sending from many addresses can no longer make every
  other request to the form slower. (#92)
